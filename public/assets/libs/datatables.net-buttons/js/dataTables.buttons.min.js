...
"none");a=e(a).addClass(g.contentClassName).attr("role","menu").appendTo(c);f.attr("aria-expanded","true");f.parents("body")[0]!==r.body&&(f=r.body.lastChild);g.collectionTitle&&c.prepend('<div className="dt-button-collection-title">'+g.collectionTitle+"</div>");t(c.insertAfter(f));var k=e(b.table().container()),d=c.css("position");"dt-container"===g.align&&(f=f.parent(),c.css("width",k.width()));if("absolute"===d){d=f.position();c.css({top:d.top+f.outerHeight(),left:d.left});var p=c.outerHeight();c.outerWidth();
